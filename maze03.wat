...
(data (offset (i32.const 0x140))
 ...
 (func $step_projectile (param $projectile_x i32) (param $projectile_y i32)
   ...
   (i32.store (i32.const 0x144) (local.get $projectile_x))
   (i32.store (i32.const 0x148) (local.get $projectile_y))
   (local.set $projectile_x (i32.const 0x144))
   (local.set $projectile_y (i32.const 0x148))
   (i32.eq (local.get $projectile_state) (i32.const 1))
   if 
   (call $collide_right (local.get $projectile_x) (local.get $projectile_y))
   if
    (i32.store (local.get $projectile_x) (i32.add (i32.load (local.get $projectile_x)) (i32.const 16)))
   else
  ...
  (i32.load(local.get $projectile_x))
  (i32.load(local.get $projectile_y))
 )
  end
)
